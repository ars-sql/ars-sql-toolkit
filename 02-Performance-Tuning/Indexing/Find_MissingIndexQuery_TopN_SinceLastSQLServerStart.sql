...
Description	: 
	- This script retrieves the top 25 missing index recommendations for the current database after last time sql server restart
	- auto-generates ready-to-use  CREATE NONCLUSTERED INDEX statements from SQL Server’s DMVs, and ranks them based on estimated performance improvement.
Manual Analysis Before Applying
	- Before implementing, you must manually check the following:
	- Query pattern – Do the suggested columns match your high-cost queries?
	- Index overlap – Is there already an existing index doing a similar job?
	- Write impact – Will this new index slow down inserts/updates?
	- Table size – Is the table large enough to justify indexing?
	- Usage recency – When was the last seek/scan (check last_user_seek)?
*/
